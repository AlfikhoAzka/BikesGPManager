...
export default function Layout({ children, currentScreen, setScreen, onMainMenu, onNewGame }) {
  const { team, budget, round, season, results, resetGame, unreadCount, isFactoryTeam, championshipPoints } = useGameStore()
      ...
      <header className="border-b border-gray-800 px-6 py-3 flex items-center gap-8">
        <div>
          ...
          <div className="text-sm text-gray-500">
            {team.manufacturer} · {team.type === 'factory' ? 'Factory' : 'Independent'}
          </div>
        </div>
        ...
        <div className="flex gap-6">
          <div className="text-center">
            ...
            <div className="text-base font-semibold text-green-400">€{budget}M</div>
          </div>
          <div className="text-center">
            <div className="text-sm text-gray-500">Round</div>
            <div className="text-base font-semibold">{round}/20</div>
          </div>
          <div className="text-center">
            <div className="text-sm text-gray-500">Points</div>
            <div className="text-base font-semibold text-yellow-400">{championshipPoints ?? 0}</div>
          </div>
          <div className="text-center">
            <div className="text-sm text-gray-500">Season</div>
            <div className="text-base font-semibold">{season}</div>
          </div>
        </div>
      </header>
      ...
      <div className="flex flex-1">
        <aside className="w-52 border-r border-gray-800 flex flex-col">
          ...
          <div className="p-4 border-t border-gray-800 space-y-2">
            <div className="text-xs text-gray-600 italic text-center">Auto-saved</div>
            <button
              onClick={() => setShowReset(true)}
              className="w-full text-sm text-gray-500 hover:text-white py-2 px-3 rounded-lg hover:bg-gray-800 transition-colors text-left"
            >
              ☰ Menu
            </button>
          </div>
